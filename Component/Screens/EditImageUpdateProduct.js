...
class EditImageUpdateProduct extends Component {
   ...
   componentDidMount() {
    ...
    AsyncStorage.getItem('@fcmtoken').then(token => {
      if (token) {
        ...
        if(this.props.navigation.state.params.imageWithID){ 
          console.log('working Image ID');
          this.state.imageID=this.props.navigation.state.params.imageWithID.images;
       ...
       console.log('images',this.state.imageID);
        ...
        this.setState({UploadedImage:this.props.navigation.state.params.imageWithID.images})
  ...
  DeleteCoverPhoto=(imageID,item)=>{
      this.showLoading();
      let productDetails=this.props.navigation.state.params.imageWithID;
  ...
  removeImageFromList(item,index) {
    ...
    console.log('image ID',JSON.stringify(this.props.navigation.state.params.imageWithID));
   if(index!==0){
    if(item.image_id){
